...
public class RuleChecker {
  private static final Set<Vector2D> allMovesForBlack =
      Set.of(
          new Vector2D(1, 0),
          new Vector2D(-1, 0),
          new Vector2D(0, 1),
  ...
  private static final Set<Vector2D> allMovesForWhite =
      Set.of(
          new Vector2D(1, 0),
          new Vector2D(-1, 0),
          new Vector2D(0, 1),
  ...
  public static boolean isValidMove(Move move, ReadOnlyCharacter character) {
    ...
    // SW
    return move.characterKind == CharacterKind.BLACK
        && move.target.x() == -1
        && move.target.y() == 1;
  }

  private static boolean isOutOfBounds(Vector2D finalPosition) {
    return finalPosition.y() < 0
        || finalPosition.y() > 7
        || finalPosition.x() < 0
        || finalPosition.x() > 7;
  }
